...
export function TokenLoginScreen({
  ...
  const [status, setStatus] = useState<"loading" | "ready" | "working" | "done" | "error">(
    "loading"
  )
  ...
  if (status === "loading" || status === "working") {
    return (
      ...
      >
        <Text color="yellow">
          {status === "loading" ? "Cargando configuración..." : "Obteniendo API key..."}
        </Text>
      </ScreenFrame>
    )
  }
  ...
  if (status === "error") {
    return (
      <ScreenFrame title="No se pudo obtener API key" help="Esc volver · Ctrl+C salir">
